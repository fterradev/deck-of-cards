...
export default function Deck (jokers, numberOfDecks = 1) {
  const cardsPerDeck = jokers ? 55 : 52;
  ...
  for (var i = cards.length; i; i--) {
    card = cards[i - 1] = Card(i - 1, Math.trunc((i - 1) / cardsPerDeck) % 2 === 0 ? "red" : "blue", numberOfDecks)
    card.setSide('back')
    card.mount($el)
  }
